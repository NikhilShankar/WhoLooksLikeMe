...
class InceptionV3MultiHead:
    def __init__(self, dataset_dir, model_dir="../facenet_keras/facenet_keras.h5", embedding_dimension=128):
        ...
        self.model = Model(inputs=base_model.input, outputs=[class_output, embedding_output])
    ...
    def train_model(self, epochs=10, batch_size=32):
        ...
        val_pred = self.model.predict(val, steps=val_steps)
        val_pred_classes = [np.argmax(val_pred[i], axis=1) for i in range(len(val_pred))]
        ...
        for i, pred in enumerate(val_pred_classes):
            report[i] = classification_report(true_labels, pred, target_names=class_names_list, output_dict=True)
        ...
        for i, r in report.items():
            report_df = pd.DataFrame(r).transpose()
            report_df.to_csv(os.path.join(output_dir, f'classification_report_head_{i}.csv'))
